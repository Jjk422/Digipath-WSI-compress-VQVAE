...
\begin{abstract}
Digital pathology whole slide images (WSIs) are large images ($\sim$30 GB/slide uncompressed) of high resolution (0.25 microns per pixel), presenting a significant data storage challenge for hospitals wishing to adopt digital pathology. Lossy compression has been adopted by scanner manufacturers to address this issue - we compare lossy JPEG compression for WSIs and investigate the Vector Quantised Variational AutoEncoder (VQVAE) as a possible alternative to reduce file size while encoding useful features in the compressed representation. \\
% This paper contains three novel contributions: a comparison of VQVAE compression performance versus standard pathology compression methods (JPEG, JPEG with no metadata and JPEG 2000 with no metadata); demonstration of generalisability and domain shift of the VQVAE network within WSI pathology data and testing of the trained networks on a real-world dataset, to compare performance against existing implementations.
We trained three VQVAE2 models from \url{https://github.com/rosinality/vq-vae-2-pytorch} on a Camelyon 2016 subset to the compression ratios of $\sim$19.2:1, $\sim$9.2:1 and $\sim$4.8:1 and tested on a Camelyon 2016 subset; a University of California (UCLA) dataset and an internal validation dataset. We then compared compression performance to ImageMagick JPEG and JPEG 2000. \\
% The PSNR ($PSNR\in R$) for a Camelyon 2016 subset (10 WSI slides) and University of California dataset were 28 dB and 28.75 dB for a compression ratio of $\sim$19.2:1 JPEG compression with metadata. For the VQVAE compression the PSNRs were 19.6±1.77 dB, 20.3±1.09 dB and 21.49±1.83 dB respectively. \\
% Structural Similarity (SSIM) considers image structure, PSNR does not. SSIM ($SSIM\in[0,1]$) results for the Camelyon 2016 subset and UCLA datasets were 0.35 and 0.35 for JPEG with exif data. The SSIM (mean±standard deviation) for the Camelyon 2016 subset, UCLA dataset and internal validation training dataset was 0.72±0.076, 0.88±0.018 and 0.80±0.060 ($\sim$19.2:1 compression ratio); 0.75±0.067, 0.87±0.023 and 0.81±0.054 ($\sim$9.2:1 compression ratio); 0.32±0.12, 0.52±0.039, 0.39±0.13 ($\sim$4.8:1 compression ratio) for the VQVAE implementation trained on the Camelyon 2016 training data subset (9 WSIs). With JPEG metadata removed from the compression the PSNR and SSIM increased to 30.03±1.34, 32.20±0.89, 30.24±1.31 and 0.88±0.025, 0.87±0.018, 0.84±0.038 for the Camelyon 2016 subset, UCLA dataset and an Internal validation set. JPEG 2000 compression without metadata outperformed all other compression methods with the PSNR and SSIM being 70.55±4.078, 80.04±2.09, 72.86±3.078 and 0.82±0.082, 0.95±0.015, 0.84±0.064 for the Camelyon 2016, UCLA and Internal validation dataset. \\
...
%     \toprule
%     \textbf{Algorithm} & \textbf{Has meta data} & \textbf{Test dataset} & \textbf{Peak signal to noise ratio (PSNR)} & \textbf{Structural Similarity (SSIM)} \\
%     \bottomrule
%     % & jpeg; meta\\
%     % \midrule
...
%     \caption{Image reconstruction quality boxplot for VQVAE with different colour augmentations.}
%   \end{figure}
...
% The poorer results for the VQVAE2 implementation on the PSNR could have been down to a lack of training of the model. The poorer results for the VQVAE implementation versus the JPEG implementations could also have been due to the use of suboptimal training hyper parameters, or possibly the use of a loss function that did not focus on the image reconstruction quality with appropriate metrics (PSNR or SSIM rather than MSE).
% % , with the latent space embedding representations being favoured or balanced over the reconstruction accuracy metrics of PSNR or SSIM.
...
% The JPEG and JPEG 2000 implementations optimise towards an SSIM or PSNR value, whereas the loss function for the trained VQVAE was focussed on embedding similarity towards the normal distribution sample with reconstruction loss being in the form of the MSE. 

% were focussed on a euclidian difference between the embeddings and the normal distribution sample, as well as the MSE loss. Therefore it may be possible to optimse the VQVAE towards attaining a good SSIM value by encorporating this within the loss function itself.
